"""Implement Auto Correct, clone from repo: https://github.com/filyp/autocorrect/tree/master"""
...
from .constants import backup_urls, ipfs_gateways, ipfs_paths, word_regexes
...
def load_from_tar(lang, file_name="word_count.json"):
    ...
    if not os.path.isfile(archive_name):
        logging.warning("dictionary for this language not found, downloading...")
        urls = [
        ...
        for url in urls:
            ...
            try:
                ...
                logging.warning(f"couldn't download {url}, trying next url...")
...
def load_from_json_file(file: str) -> dict:
    with open(file) as f:
        try:
            ...
            logging.warning("JSON File wrong format!")
...
class Speller:
    ...
    def autocorrect_word(self, word):
        ...
        if word[0].isupper():
            decapitated = word[0].lower() + word[1:]
            candidates += self.get_candidates(decapitated)
