...
def recheck_order_status(apps, schema_editor):
    ...
    for order in list_order:
        ...
        if order.status == QueueStatus.Cancelled.value:
            if QueueStatus.Cancelled.value.upper() not in status_history:
                new_history = RetailerPurchaseOrderHistory(
                    status=QueueStatus.Cancelled.value, order=order
                )
                new_history.save()
        elif order.status != QueueStatus.Cancelled.value:
            if QueueStatus.Cancelled.value.upper() in status_history:
                order.status = QueueStatus.Cancelled.value
                # delete history ship
                list_status_delete = [
                    QueueStatus.Shipped.value,
                ...
                ship_history = list_order_history_for_order.filter(
                    status__in=list_status_delete
                )
                ship_history.delete()
            else:
                ...
                if is_partly_ship is True:
                    ...
                    if check_full_shipped is True:
                        ...
                        if invoice_for_order is not None:
                            ...
                            if invoice_for_order.doc_number is not None:
                                ...
                                if (
                                    QueueStatus.Invoiced.value.upper()
                    ...
                    # if order is partly ship
                        ...
                        if (
                            QueueStatus.Partly_Shipped.value.upper()
                            not in status_history
                        ):
                            new_history = RetailerPurchaseOrderHistory(
                                status=QueueStatus.Shipped.value, order=order
...
class Migration(migrations.Migration):
    dependencies = [
        ("retailer_purchase_orders", "0024_update_status_for_order"),
    ]
